...
export class Thread extends Component {
    ...
    submitHandler = (e) => {
        ...
        axios.post(`http://localhost:3001/conversations/${this.state.id2}/${this.state.sub}`, data)
        .then((response)=>{
    ...
    componentDidMount(){
        axios.get(`http://localhost:3001/conversations/${this.state.id2}/${this.state.sub}`)
        .then((response) => {
