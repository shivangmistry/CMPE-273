...
export class Quizinfo extends Component {
    ...
    componentDidMount(){
        axios.get(`http://localhost:3001/course/${this.state.cid}/quiz/${this.state.qname}`)
        .then((response)=>{
            ...
            else if(response.data.message==="prohibited"){
    ...
    submithandler = (e)=>{
        ...
        axios.post(`http://localhost:3001/course/${this.state.cid}/quiz/${this.state.qname}`,data)
        .then((response)=>{
