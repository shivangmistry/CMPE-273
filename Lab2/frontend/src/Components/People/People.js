...
export class People extends Component {
    ...
    nxtHandler = (e) => {
        ...
        axios.get(rooturl+`/people?page=${t}&limit=${this.state.limit}`)
        .then((response) => {
            ...
            if(response.data.data.message==="error"){
                ...
                // alert("Next page received.")
                if (!response.data.data.data[0]){
                    this.paginate(-1)
                }
                else{
                    this.setState({ people: response.data.data.data})
                }
            }
        })
    }
    ...
    componentDidMount() {
        axios.get(rooturl+`/people?page=0&limit=${this.state.limit}`)
            .then((response) => {
                console.log(response.data)
                            ...
                            <div className="peepscontainer">
                                {people.map((p, index) =>{
                                    return <Link to={`/profile/${p._id}`} key={index}><div  className="row peeps">
                                        ...
                                        <div className="col-4 peoplerole">
