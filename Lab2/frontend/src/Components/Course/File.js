...
export class File extends Component {
  ...
  componentDidMount(){
    axios.get(rooturl+`/course/${this.state.cid}/file`)
    .then((response)=>{
      console.log(response.data);
      if(response.data.data.message==="error"){
        alert("Something went wrong.");
        this.props.history.push(`/course/${this.state.cid}/home`);
      }
  ...
  submitHandler = (e) =>{
    ...
    axios.post(rooturl+`/course/${this.state.cid}/file`,data)
    .then((response)=>{
          ...
          <div className="row">
            ...
            <div className="col-9 coursecolumn">
              ...
              {files.map((file,index)=>{
                return <div key={index} className="filelist" ><a href={rooturl+`/uploads/${file.fpath}`} download target="_blank" rel="noopener noreferrer" >{file.fname}</a></div>
              })}
              {(!isStudent)
