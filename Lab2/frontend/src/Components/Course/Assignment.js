...
export class Assignment extends Component {
  ...
  componentDidMount() {
    axios.get(rooturl+`/course/${this.state.cid}/assignment`)
      .then((response) => {
        console.log(response.data);
        if(response.data.data.message==="error"){
          alert("Something went wrong.");
          this.props.history.push(rooturl+`/course/${this.state.cid}/home`)
        }
        else if(response.data.data.message==="success"){
          this.setState({
            ass: response.data.data.data
          })
        }
      })
  }
        ...
        <div className="pageContent">
          <div className="row">
            ...
            <div className="col-9 coursecolumn">
              <h3>Assignment</h3><br />
              {ass.map((a,index)=>{
                return <div className="filelist" key={index} style={{padding:"10px"}}><a href={rooturl+`/uploads/${a.aspath}`} download target="_blank" rel="noopener noreferrer" >{a.asname}</a>
                  {(isStudent)
