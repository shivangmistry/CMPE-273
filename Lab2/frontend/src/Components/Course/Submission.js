...
export class Submission extends Component {
    ...
    componentDidMount(){
      axios.get(rooturl+`/course/${this.state.cid}/assignment/${this.state.asname}`)
      .then((response)=>{
        // console.log(response.data);
        if(response.data.data.message==="error"){
          alert("Something went wrong.")
          this.props.history.push(rooturl+`/course/${this.state.cid}/assignment`);
        }
        else if(response.data.data.message==="success"){
          this.setState({
            subs: response.data.data.data
          })
        }
      });
    }
    ...
    submitHandler = (e) => {
        ...
        // console.log(data);
        axios.post(rooturl+`/course/${this.state.cid}/assignment/${this.state.asname}`, data)
            .then((response) => {
                ...
                {subs.map((sub,index)=>{
                  return <table key={index} className="subtable"><tbody>
                    <tr>
                      <td>{sub.sid}</td>
                      <td>{sub.sname}</td>
                      <td style={{width:"350px"}}><a href={rooturl+`/uploads/${sub.subpath}`} download target="_blank" rel="noopener noreferrer">{sub.subname}</a></td>
